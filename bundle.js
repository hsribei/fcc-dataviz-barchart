...
var d3 = _interopRequireWildcard(__webpack_require__(177));
...
function makeABarChart(json) {
  ...
  function formatDate(date) {
    var str = d3.timeFormat("%Y - %B")(date) + " (Q";
  ...
  var xBars = d3.scaleBand().range([0, width]).paddingInner(0.2);
  xBars.domain(data.map(function (d) {
    return d.date;
  }));
  var svg = d3.select("div#d3").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // Add the X Axis
  ...
  svg.append("g").attr("class", "y axis").call(yAxis).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".71em").style("text-anchor", "end").text("Gross Domestic Product, USA"); // Add the value line path
  ...
  svg.append("path").data([data]).attr("class", "line").attr("d", valueLine); // Define the div for the tooltip

  var div = d3.select("body").append("div").attr("class", "tooltip").style("opacity", 0); // Add the bars

  svg.selectAll(".bar").data(data).enter().append("rect").attr("class", "bar").attr("x", function (d) {
    return xBars(d.date);
  }).attr("width", xBars.bandwidth()).attr("y", function (d) {
    return y(d.GDP);
  }).attr("height", function (d) {
    return height - y(d.GDP);
  }).on("mouseover", function (d) {
    div.transition().duration(200).style("opacity", 0.95);
    div.html(d.GDP.toLocaleString(undefined, {
      style: "currency",
      currency: "USD"
    }) + " Billion" + "<br />" + '<span class="gdp">' + formatDate(d.date) + "</span>").style("left", d3.event.pageX + "px").style("top", d3.event.pageY + "px");
  });
